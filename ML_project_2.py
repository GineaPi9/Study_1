...
for train_index, test_index in kfold.split(Param):  # feautres 데이터를 위에서 지정한 kfold 숫자로 분할
    Param_train, Param_test = Param[train_index], Param[test_index]
    Target_train, Target_test = Target[train_index], Target[test_index]
    
    dt_clf.fit(Param_train, Target_train)
    pred = dt_clf.predict(Param_test)
    n_iter += 1
    
    accuracy = np.round(accuracy_score(Target_test, pred), 4) # 소수점 4자리 반올림
    train_size = Param_train.shape[0]
    test_size = Param_test.shape[0]
...
# plt.scatter(LinReg_Compare_Data.index, LinReg_Compare_Data['Predicted_Value'], marker='x', color='black')
# plt.scatter(LinReg_Compare_Data.index, LinReg_Compare_Data['Actual_Value'], color='crimson',  alpha=0.7)
...
# # 예측 CMEDV와 실제 CMEDV(test data) 비교 그래프
# plt.figure(figsize=(7, 7))
# plt.scatter(RidgeReg_Compare_Data.index, RidgeReg_Compare_Data['Predicted_Value'], marker='x', color='black')
# plt.scatter(RidgeReg_Compare_Data.index, RidgeReg_Compare_Data['Actual_Value'], color='darkorange',  alpha=0.7)
...
# 모델을 이용해 예측한 CMEDV와 실제 test data CMEDV 비교를 위한 Data 생성
DecTreeReg_Compare_Data = pd.DataFrame({'Actual_Value':Target_test, 'Predicted_Value':pred}) # Data frame 1열 : 실제 data, 2열 : 예측 data
...
# 예측 CMEDV와 실제 CMEDV(test data) 비교 그래프
plt.figure(figsize=(7, 7))
plt.scatter(DecTreeReg_Compare_Data.index, DecTreeReg_Compare_Data['Predicted_Value'], marker='x', color='black')
plt.scatter(DecTreeReg_Compare_Data.index, DecTreeReg_Compare_Data['Actual_Value'], color='blue',  alpha=0.7)
